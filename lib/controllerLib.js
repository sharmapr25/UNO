...
exports.sendUpdatedData = function(request, response, game){
  if(game) {
    ...
    if(game.isGameStarted){
      startUno(game); 
      response.send(JSON.stringify({isStarted : true,location:'/htmlFiles/unoTable.html'}));
    };
    response.send(JSON.stringify({isStarted : game.isGameStarted ,noOfPlayers : game.usersInformation.length}))
  }
...
var setCookie = function(cookieHeader){
 ...
 var generateTable  = function(userInfo) {
  ...
  var linkToHome = "<button onclick = 'resetCookie()'>Play Again</button>"
  var endData = '</body></html>';
  return "<table id='table'>"+greetText+tableHead+user_info.join('')+"</table>" +linkToHome+ endData;
};
...
exports.handle_say_uno = function(request, response, game){
  ...
  console.log("player Name in handle_say_uno",playerName);
  ...
  game.said_UNO_registry.forEach(function(player){
    if(player.said_uno == true){
      if(game.user_cards[player.name].length != 1)
        ...
        console.log("setting say uno must be false and player",player.said_uno,player);
    };
  });
};
  ...
  for(var i = 0; i < playersCardInfo.length; i++){
    if(playersCardInfo[i].noOfCards == 1){
      ...
      for(var j = 0; j < game.said_UNO_registry.length; j++){
        if(game.said_UNO_registry[j].said_uno == false) {
            givePenalty(game.said_UNO_registry[j].name, 2, game);
...
exports.handle_join_game = function(request, response, game, joinUserName){
    ...
    console.log("game in handle_join_game in controller lib",game);
    response.send('/htmlFiles/loadingPage.html');   
};
