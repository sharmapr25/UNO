...
exports.isValid = function(userData) {
  if(userData.name){
    return (userData.name.split(' ').length == 1 && userData.no_of_players <= 6 && userData.no_of_players > 0) ? true : false;
  }
  return false;
}

exports.isValidForJoin = function(userData){
    return (userData.name && userData.name.split(' ').length == 1 ) ? true : false;
}
...
exports.createdNewGameId = function(userInfo, gameId, listOfGame, game){
	listOfGame[gameId] = new game(gameId,userInfo.no_of_players);
	addUser(userInfo.name, listOfGame[gameId]);
};
...
var generateDiv = function(userInfo){
  ...
  var winner = '';
  userInfo.forEach(function(eachUser){
    if(eachUser.points == 0)
      winner = eachUser.name;
  });

  var greetText = '<div class = "congratulate spaceBetweenTwoClass">Congratulations!!!</div>';
  var message = '<div class = "message spaceBetweenTwoClass">'+winner+' won the game</div>';
  ...
  return "<div id = 'winner'>"+greetText+message+list+"</div>"+"<div id = 'list'>"+headingOfList+"<div class = 'userInfo'>"+user_info+"</div>"+"</div>"+linkToHome+endData;
}
...
exports.sendAllInformationOfTable = function(request, response, game){
  if(!game)
    ...
    dataToSend.numberOfPlayers = game.no_of_players;
...
var givePenaltyCardsTwoNextPlayer = function(penalty, game){
  if(game.user_cards[game.players.currentPlayer].length !=0){
    console.log("next player",game.players.nextPlayer);
    console.log("next player's cards",game.user_cards[game.players.nextPlayer]);
  ...
  switch (cardPlayed.playedCard.speciality){
    ...
    case 'DrawTwo':
      ...
      if(!doesNextPlayerHavePlustwo(game)){
        console.log("game.plus_two_cards_count",game.plus_two_cards_count);
        givePenaltyCardsTwoNextPlayer(game.plus_two_cards_count, game);
        game.players.changePlayersTurn();
        console.log("dchakhgd");
...
var drawCardsFromDeck = function(noOfCards, game){
  ...
  console.log("card in drawCardsFromDeck",cards);
...
exports.handle_pass_turn_request = function(request, response, game){
  if(setCookie(request.headers.cookie) == game.players.currentPlayer){
    game.players.changePlayersTurn();
    game.currentPlayer = game.players.currentPlayer;
    response.statusCode = 200;
