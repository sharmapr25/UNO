...
exports.isValid = function(userData) {
  if(userData.name){
    // console.log(userData.name.split(' ').length == 1 && userData.no_of_players <= 6 && userData.no_of_players > 0,userData.name.split(' '),userData.name.split(' ').length,userData.no_of_players <= 6 ,userData.no_of_players > 0)
...
var generateDiv = function(userInfo){
  ...
  var winner = userInfo.forEach(function(eachUser){
    if(eachUser.points == 0)
      "<div id = 'winnerName'>"+eachUser.name+"</div>";
  });
